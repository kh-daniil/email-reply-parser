...
class EmailParser {
  ...
  fixBrokenSignatures(text) {
    ...
    for (const regex of RegexList.quoteHeadersRegex) {
  ...
  isQuoteHeader(line) {
    ...
    return RegexList.quoteHeadersRegex.some((regex) => regex.test(text));
  }

  isSignature(line) {
    let text = this.stringReverse(line);

    return RegexList.signatureRegex.some((regex) => regex.test(text));
  }
  ...
  isEmpty(fragment) {
    return fragment.lines.every(line => line === "");
  }
